    ...
    return [
        ...
        for _, player in pd.concat(
            [
                ...
                PLAYERS_DF[PLAYERS_DF["position_name"] == "MIDFIELDER"].sample(
                    midfielders
                ),
...
def test_team_initialization():
    ...
    with pytest.raises(UnsupportedLineUp):
        _ = Team(
            players=_get_team("3-3-4"), substitutes=_get_team("2-2-2"), line_up="3-3-4"
        )
    _ = Team(
        players=_get_team("4-3-3"), substitutes=_get_team("2-2-2"), line_up="4-3-3"
    )
...
def test_team_points():
    ...
    team = Team(
        players=_get_team("4-4-2"), substitutes=_get_team("2-2-2"), line_up="4-4-2"
    )
